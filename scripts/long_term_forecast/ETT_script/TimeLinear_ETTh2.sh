...
python -u run.py \
  ...
  --loss mse \
  --seed $seed \
  --itr 1 | tee -a ./log/TimeLinear/etth2/$seq_len'_'192.txt
done
done
done
...
python -u run.py \
  ...
  --loss mse \
  --seed $seed \
  --itr 1 | tee -a ./log/TimeLinear/etth2/$seq_len'_'336.txt
done
done
done
...
python -u run.py \
  ...
  --loss mse \
  --seed $seed \
  --itr 1 | tee -a ./log/TimeLinear/etth2/$seq_len'_'720.txt
done
done
done
